...
from models.experimental import attempt_load, get_feas_by_hook, get_s_feas_by_hook
...
from utils.loss import ComputeLoss, compute_kd_output_loss, EFKD, EFTLoss
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if opt.kd:
        ...
        if teacher_model.get("model", None) is not None:
            teacher_model = teacher_model["model"]  # 获取model参数
    ...
    theta = torch.ones(1, device=device)  # 各个损失的自适应权重参数
    ...
    if pretrained:
        if 'theta' in ckpt.keys():
            theta.data = ckpt['theta'].data
            theta = theta.to(device)
        if 'beta' in ckpt.keys():
            beta.data = ckpt['beta'].data
            beta = beta.to(device)
        del ckpt

    sigmod_fun = nn.Sigmoid()
    sigmod_fun_2 = nn.Sigmoid()
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                s_f = get_s_feas_by_hook(model)  # hook机制
                pred = model(imgs)  # forward

                # # graph = make_dot(pred, params=dict(model.named_parameters()), )  # 生成计算图结构表示
                # # graph=make_dot(out,params=dict(model.named_parameters()),show_attrs=True,show_saved=True)  # 生成计算图结构表示
                # graph=make_dot(pred)  # 生成计算图结构表示
                ...
                if opt.kd:
                    with torch.no_grad():
                        t_f = get_feas_by_hook(teacher_model)
                ...
                if opt.kd:
                    kdloss = compute_kd_output_loss(pred, teacher_pred, model, opt.kd_loss_selected, opt.temperature)
                    del teacher_pred
                else:
                    kdloss = 0

                theta = sigmod_fun(theta)
                beta = sigmod_fun_2(beta)
                # theta = torch.tensor(1 / (1 + math.exp(-theta)))  # sigmoid函数
                # theta.requires_grad = True
                # beta = torch.tensor(1 / (1 + math.exp(-beta)))
                # beta.requires_grad = True

                # loss += opt.alpha * kdloss + opt.beta * ftloss  # 后面可以把opt.alpha和opt.beta改成自适应参数，并通过sigmoid函数限制参数取值范围
                # loss_total = loss + (1-theta) * kdloss + theta * ftloss  # 后面可以把opt.alpha和opt.beta改成自适应参数，并通过sigmoid函数限制参数取值范围
                loss_total = loss + beta * kdloss + theta * ftloss  # 后面可以把opt.alpha和opt.beta改成自适应参数，并通过sigmoid函数限制参数取值范围

                kdloss_item = kdloss.detach()  # kdloss_item不参与反向更新
                ftloss_item = ftloss.detach()
            ...
            scaler.scale(loss_total).backward(retain_graph=True)
    ...
    print("theta = " + str(theta) + "---" + "beta" + str(beta))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--teacher_weight', type=str, default='best.pt', help='initial teacher_weight path')
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'runs/train/yolov5x_d1.33_w1.25_PruningBaseline_d0.1_w0.03_layer_134_kd5_qiou0.2_theta_beta_C3Cut/weights/best.pt', help='initial weights path')  # 接着上一次的继续训练
    parser.add_argument('--cfg', type=str, default='simplest_yolov5s_pruning.yaml', help='model.yaml path')
