...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=[0.5, 0.5],
                 ratio=[1.0 for _ in range(10)]):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        # c_ = int(c2 * e)  # hidden channels
        # print('*****************************', c2)
        c_1 = int(c2 * e[0])
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, exp_size, k=3, se_ratio=0, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.conv = nn.Sequential(GhostConv(c1, exp_size, 1, 1),  # pw
                                  ...
                                  SqueezeExcite(exp_size, se_ratio=se_ratio) if (
                                                                                            se_ratio and se_ratio) > 0 else nn.Sequential(),
                                  GhostConv(exp_size, c2, 1, 1, act=False))  # pw-linear
        self.shortcut = nn.Identity() if (c1 == c2 and s == 1) else nn.Sequential(DWConv2(c1, c1, k, s),
